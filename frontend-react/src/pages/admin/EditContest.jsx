...
function EditContest() {
  ...
  useEffect(() => {
    const fetchContest = async () => {
      try {
        ...
        const response = await adminApi.getContest(contestId)
        ...
        // IP 범위 변환
        let allowed_ip_ranges = data.allowed_ip_ranges || []
        if (allowed_ip_ranges.length === 0) {
          allowed_ip_ranges = [{ value: '' }]
        } else {
          allowed_ip_ranges = allowed_ip_ranges.map((value) => ({ value }))
        }
        ...
        setFormData({
          ...
          rule_type: data.rule_type || 'ACM',
          real_time_rank: data.real_time_rank !== undefined ? data.real_time_rank : true,
          visible: data.visible !== undefined ? data.visible : true,
          allow_paste: data.allow_paste !== undefined ? data.allow_paste : true,
          allowed_ip_ranges,
        })
      } catch (error) {
        const message = handleApiError('Fetch Contest', error, t('Fetch_Failed'))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requestData = {
        ...
        allow_paste: formData.allow_paste,
        allowed_ip_ranges,
      }

      await adminApi.updateContest(requestData)

      // 성공 시 대회 목록으로 이동
      navigate('/admin/contest', { state: { refresh: true } })
    } catch (error) {
      const message = handleApiError('Update Contest', error, t('Update_Failed'))
    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          <p className="text-red-700 dark:text-red-400">{errors.fetch}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">{t('Edit_Contest')}</h1>
        <p className="text-muted-foreground mt-2">{t('Edit_Contest_Description')}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-4">
          ...
          >
            {loading ? t('Updating') : t('Update_Contest')}
          </button>
        </div>
      </form>
    </div>
