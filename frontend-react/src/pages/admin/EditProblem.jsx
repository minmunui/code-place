...
const EditProblem = () => {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        const response = await api.get('/admin/problem', { params: { id: problemId } })
        ...
        setFormData({
          ...
          difficulty: problem.difficulty || 'Low',
          visible: problem.visible !== undefined ? problem.visible : true,
          share_submission: problem.share_submission || false,
          field: problem.field !== undefined ? problem.field : '',
          tags: problem.tags || [],
          languages: problem.languages || [],
          samples: problem.samples && problem.samples.length > 0 ? problem.samples : [{ input: '', output: '' }],
          hint: problem.hint || '',
          template: problem.template || {},
          spj: problem.spj || false,
          spj_language: problem.spj_language || 'C',
          spj_code: problem.spj_code || '',
          spj_compile_ok: problem.spj_compile_ok || false,
          test_case_id: problem.test_case_id || '',
          test_case_score: problem.test_case_score || [],
          rule_type: problem.rule_type || 'ACM',
        ...
        alert(t('Failed_To_Load_Problem') || 'Failed to load problem')
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await api.put('/admin/problem', formData)
      alert(t('Update_Succeeded') || 'Problem updated successfully')
      navigate('/admin/problems')
    } catch (error) {
      const message = handleApiError('Update Problem', error, t('Update_Failed'))
      alert(message)
    } finally {
      setLoading(false)
    }
    ...
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-4">
          ...
          >
            {loading ? t('Updating') : t('Update_Problem')}
          </button>
        </div>
      </form>
    </div>
