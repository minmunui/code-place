...
/**
 * 대회 문제 상세 페이지
 * 대회의 특정 문제를 표시하고 코드를 제출할 수 있습니다.
 */
function ContestProblemDetail() {
  ...
  /**
   * 대회 문제 상세 정보를 서버에서 불러옵니다.
   * 기본 언어를 설정하고 코드 템플릿이 있으면 에디터에 로드합니다.
   */
  const loadProblem = async () => {
    try {
      setLoading(true)
      const response = await api.getContestProblem(contestID, problemID)
      ...
      console.error('Failed to load contest problem:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true)
      const response = await api.submitCode(problem.id, language, code, contestID)
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <div className="text-muted-foreground mb-4">문제를 찾을 수 없습니다.</div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {activeTab === 'problem' ? (
            <div className="space-y-6">
              ...
              {/* 문제 설명 */}
              <div>
                <h2 className="text-xl font-bold text-foreground mb-2">설명</h2>
                <div
              ...
              {/* 입력 */}
              <div>
                <h2 className="text-xl font-bold text-foreground mb-2">입력</h2>
                <div
                  className="text-foreground prose prose-sm max-w-none dark:prose-invert"
              ...
              {/* 출력 */}
              <div>
                <h2 className="text-xl font-bold text-foreground mb-2">출력</h2>
                <div
                  className="text-foreground prose prose-sm max-w-none dark:prose-invert"
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-foreground">
                  <li>
                    시간 제한: <code className="text-primary">{problem.time_limit}ms</code>
                  </li>
                  <li>
                    메모리 제한:{' '}
              ...
              {problem.hint && (
                <div>
                  <h2 className="text-xl font-bold text-foreground mb-2">힌트</h2>
